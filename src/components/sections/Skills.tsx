...
// Define Skill interface for TypeScript
interface Skill {
  name: string;
  desc: string;
  src: string;
...
// Skills array as readonly tuple
const skills = [
  {
    name: 'Next.js',
    desc: 'Modern SSR & ISR',
  ...
  {
    name: 'Nest.js',
    desc: 'TypeScript backend',
    src: '/blogs/nestjs',
    icon: Server,
  },
  {
    name: 'Fastify',
    desc: 'Fast Node.js APIs',
    src: '/blogs/fastify',
    icon: Server,
  },
  {
    name: 'Express.js',
    desc: 'Robust Node.js framework',
    src: '/blogs/express',
    icon: Server,
  },
  {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        {skills.map((skill) => {
          const tooltipId = `tooltip-${skill.name.toLowerCase().replace('.', '-')}`;
          const tooltipText = useMemo(
            () =>
              skill.src === '#'
                ? `Coming soon: ${skill.name} guide`
                : `Read my ${skill.name} blog`,
          ...
          return (
            ...
            >
              ...
              <span
                id={tooltipId}
                className="absolute hidden group-hover:block top-[-2.5rem] left-1/2 transform -translate-x-1/2 bg-gray-800 dark:bg-gray-700 text-white dark:text-gray-200 text-xs rounded py-1 px-2 whitespace-nowrap animate-fade-in"
              >
                {tooltipText}
              </span>
            </article>
